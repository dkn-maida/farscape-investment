...
data = yf.download('BTC-USD', start='2008-01-01', end='2023-07-26')

# Calculate the 10-day high and 100-day EMA
data['10_day_high'] = data['Close'].rolling(window=7).max()
data['ema100'] = data['Close'].ewm(span=50, adjust=False).mean()

# Create a signal when the close price crosses the 10_day_high and is above the EMA100
data['buy_signal'] = np.where((data['Close'] > data['10_day_high'].shift()) & (data['Close'] > data['ema100']), 1, 0)
...
data['sell_signal'] = np.where((data['take_profit_signal'] | data['exit_signal']) & (data['Close'] > data['ema100']), 1, 0).astype(int)
...
plt.plot(data['strategy_cumulative_returns'], label='10 Day High with EMA100 filter Strategy')
plt.legend()
plt.show()
